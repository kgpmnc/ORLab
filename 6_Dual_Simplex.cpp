...
float mat[20][20] = {{-0.33,0.33,1.66}, {1.33,-0.33,1.33}, {-0.67,-0.33,-0.67}, {1/3,11/3,41/3}};
float b[20], temp[20][20], constants[20];
int R, C;
int rank1, rank2, i, j, k, in_var, var = 2, eqn = 3;
...
void printTableau(){
    ...
    cout<<"Tableau no."<<(++no)<<endl;
     for(int i = 0 ; i <=eqn ; i++){
          for(int j = 0 ; j <= var; j++){
               cout << fixed<<setw(7)<<setprecision(2)<<std::setfill(' ') <<mat[i][j] << " ";
          }
          cout << endl;
     }
     cout<<endl;
